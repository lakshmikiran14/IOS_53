...
'''
    ...
    @credits Sudheer - for explaining his approach and how to use os.path and
    mimetype.
'''
...
class HTTPServer:
    def __init__(self, IP, port):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP) as self.s:
            ...
            while True:
                ...
                if conn:
                	x = threading.Thread(target = self.handle_thread, args = (conn,))
                	x.start()
    ...
    def get_data(self, uri):
        ...
        t = uri.split('/')
        if 'bin' in t:
            uri = os.getcwd().split('/Server')[0] + uri
            ...
            if pid:
                # parent process
                time.sleep(2)
                p, status = os.waitpid(0, os.WNOHANG)
                if p == 0:
                    os.kill(pid, signal.SIGSTOP)
                    data = '<h3>Terminated due to time out</h3>'
